import { Game, moveElementToFirst } from "./index.mjs";
...
export class Self {
    ...
    /** The performance.now() timestamp when the gameplay started */ startTime = null;
    /** The last time IGEs were flushed */ lastIGEFlush = performance.now();
    /** The tick function called every game tick */ tick;
   ...
   */ destroy() {
    ...
    /** Initialize the client's game. You don't need to call this manually. */ init() {
    ...
    #flushFrames() {
        let returnFrames = this.#frameQueue.filter((f)=>f.frame <= this.engine.frame);
        this.#frameQueue = this.#frameQueue.filter((f)=>f.frame > this.engine.frame);
    ...
    async #tickGame() {
        ...
        if (this.tick) {
            try {
                ...
                if (res.keys) this.keyQueue.push(...res.keys.filter((k, idx)=>{
                    if (!isValidObject(k) || !(k.type === "keydown" || k.type === "keyup") || typeof k.frame !== "number" || isNaN(k.frame) || k.frame < this.engine.frame || !isValidObject(k.data) || ![
                        "moveLeft",
                        "moveRight",
                        "hardDrop",
                        "hold",
                        ...
                        Game.log(`Invalid key event at index ${idx} passed on frame ${this.engine.frame}:\n${JSON.stringify(k, null, 2)}`, {
                            level: "error"
                        });
                        return false;
                    }
                ...
                if (res.runAfter) runAfter.push(...res.runAfter.filter((ra, idx)=>{
                    if (typeof ra !== "function") {
                        Game.log(`Invalid runAfter callback at index ${idx} passed on frame ${this.engine.frame}.`, {
                            level: "warning"
                        });
                        return false;
                    }
        ...
        try {
            ...
            this.#flushIGEs();
            const target = (this.engine.frame + 1) / Game.fps * 1000 - (performance.now() - this.startTime);
            if (target <= -2000 && !this.#slowTickWarning) {
                Game.log(`Triangle.js is lagging behind by more than 2 seconds! Your \`tick\` function is likely taking too long to execute.`, {
                    level: "warning"
                });
    ...
    #flushIGEs() {
        if (this.igesPaused && this.lastIGEFlush + Self.maxIGETimeout > performance.now()) return;
