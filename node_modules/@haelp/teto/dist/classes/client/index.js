...
const _hook = _export_star(require("./hook"), exports);
...
class Client {
   ...
   */ emit;
    /** @hideconstructor */ constructor(token, sessionID, ribbon, me, userAgent = _utils.CONSTANTS.userAgent, gameOptions){
        this.token = token;
        this.ribbon = ribbon;
   ...
   */ static async create(options) {
        ...
        const handling = options.game?.handling || {
            arr: 0,
            cancel: false,
            das: 5,
            dcd: 0,
        ...
        const data = await new Promise((resolve, reject)=>{
            ...
            ribbon.emitter.once("client.fail", (e)=>{
                clearTimeout(t);
                reject("Failed to connect: " + (e.stack ?? e.message)?.replace("Error: ", ""));
            });
            ribbon.open();
        });
        const client = new Client(token, sessionID, ribbon, await me, options.userAgent || _utils.CONSTANTS.userAgent, {
            handling,
   ...
   */ wrap(event, data, listen, error = [
    ...
    hook() {
        return new _hook.Hook(this.ribbon.emitter);
    }
    /** @hidden */ #init() {
   ...
   */ async reconnect() {
    ...
    /** The client's current handling. Do not change the client's handling while in a room. */ get handling() {
    ...
    /** The client's spectating strategy. */ get spectatingStrategy() {
        return this.#spectatingStrategy;
    }
    set spectatingStrategy(strategy) {
