...
_export(exports, {
    ...
    get moveElementToFirst () {
        return moveElementToFirst;
    }
});
const _engine = require("../../engine");
const _constants = require("../../engine/constants");
const _player = _export_star(require("./player"), exports);
const _self = _export_star(require("./self"), exports);
...
class Game {
    #client;
    #strategy;
    #spectatingTimeout = null;
    #spectateWarningCounter = 0;
    /** Data on all players in game, including the client. Note that the client's engine data is only the data acknowledged by the server, not the most recent gameplay information. */ players = [];
    /** The raw game config for all players, (possibly) including the client's own game config */ rawPlayers;
    /** The client's own game handler */ self;
    get opponents() {
        return this.players.filter((p)=>p.userid !== this.#client.user.id);
    }
    /** The Frames Per Second of the TETR.IO engine */ static fps = 60;
    /** @hideconstructor */ constructor(client, players){
        this.#client = client;
        this.#strategy = this.#client.spectatingStrategy;
        const self = players.find((p)=>p.userid === client.user.id);
        if (self) {
            this.self = new _self.Self(client, players);
        }
   ...
   */ destroy() {
    ...
    #init() {
        ...
        this.players = this.rawPlayers.map((p)=>new _player.Player(this.#client, this.#strategy, p, this.rawPlayers));
        this.#spectatingTimeout = setTimeout(this.#tick.bind(this), 1000 / Game.fps);
    }
    #tick() {
        ...
        if (performance.now() - tickStart > 1000 / Game.fps - 1) {
            if (this.#spectateWarningCounter++ === 5) {
                Game.log("Spectating is falling behind! You are spectating too many players. Consider reducing the number of players you are spectating to improve performance.");
            }
        } else {
            this.#spectateWarningCounter = 0;
        }
        this.#spectatingTimeout = setTimeout(this.#tick.bind(this), Math.max(0, 1000 / Game.fps - (performance.now() - tickStart)));
    }
    #spectate(player) {
        return (typeof player === "number" ? this.players.find((p)=>p.gameid === player) : player)?.spectate() ?? Promise.reject("invalid target:");
    }
    #unspectate(player) {
        const target = typeof player === "number" ? this.players.find((p)=>p.gameid === player) : player;
    ...
    spectate(targets) {
        ...
        if (targets === "all") {
            ...
            if (typeof targets[0] === "string") {
                const useridTargets = targets;
                players = useridTargets.map((uid)=>this.players.find((p)=>p.userid === uid) ?? -1);
            } else {
                const gameidTargets = targets;
                players = gameidTargets.map((gid)=>this.players.find((p)=>p.gameid === gid) ?? -1);
            }
        } else {
            return Promise.reject('Invalid spectate targets: must be "all" | number[] | string[]');
        }
    ...
    unspectate(targets) {
        ...
        if (targets === "all") {
            ...
            if (typeof targets[0] === "string") {
                const useridTargets = targets;
                players = useridTargets.map((uid)=>this.players.find((p)=>p.userid === uid) ?? null);
            } else {
                const gameidTargets = targets;
                players = gameidTargets.map((gid)=>this.players.find((p)=>p.gameid === gid) ?? null);
            }
        } else {
            throw new Error('Invalid spectate targets: must be "all" | number[] | string[]');
        }
    ...
    /**
   * @internal
   */ static createEngine(options, gameid, players) {
    ...
    /**
   * @internal
   */ static snapshotFromState(frame, config, state) {
        return {
            // TODO: actual connected board
            board: state.board.toReversed().map((row)=>row.map((square)=>square ? {
                        mino: square,
                        connections: _engine.BoardConnections.TOP | _engine.BoardConnections.RIGHT | _engine.BoardConnections.BOTTOM | _engine.BoardConnections.LEFT
                    } : null)),
            falling: {
                aox: 0,
                aoy: 0,
            ...
            garbage: {
                ...
                queue: state.impendingdamage.map((g)=>({
                        amount: g.amt,
                        cid: g.cid,
                        gameid: g.gameid,
                        size: g.size,
                        confirmed: !!state.waitingframes.find((wf)=>wf.type === "incoming-attack-hit" && wf.data === g.id),
                        frame: state.waitingframes.find((wf)=>wf.type === "incoming-attack-hit" && wf.data === g.id)?.target ?? Infinity - config.garbage.garbage.speed
                    })),
                seed: state.rngex,
            ...
            ige: {
                ...
                ].map((p)=>parseInt(p)).map((p)=>[
                        p,
                        {
                            incoming: state.garbageacknowledgements.incoming[p] ?? 0,
                            outgoing: state.garbageacknowledgements.outgoing[p]?.map((o)=>({
                                    iid: o.iid,
                                    amount: o.amt
                                })) ?? []
                        }
            ...
            lastSpin: state.falling.flags & _constants.constants.flags.ROTATION_SPIN ? "normal" : !(~state.falling.flags & (_constants.constants.flags.ROTATION_SPIN | _constants.constants.flags.ROTATION_MINI)) ? "mini" : "none",
