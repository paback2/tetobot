...
export class Social {
    ...
    /** @hideconstructor */ constructor(client, config, init){
        ...
        if ("config" in init) {
            this.friends = init.friends.map((snapshot)=>Relationship.fromSnapshot(snapshot, this, client));
            this.other = init.others.map((snapshot)=>Relationship.fromSnapshot(snapshot, this, client));
        } else {
    ...
    #init() {
        ...
        this.#client.on("social.dm", async (raw)=>{
            ...
            if (!user) {
                console.warn(`${chalk.yellowBright("[Triangle.js]")}: Failed to load user data for ${target}. 'client.dm' event discareded.`);
                user = this.get({
                    id: dm.data.user
                });
                if (this.config.autoLoadDMs) await user.loadDms();
